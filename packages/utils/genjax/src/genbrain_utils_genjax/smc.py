...
def maybe_resample(key, log_weights_raw, ess_threshold):
    ...
    ess = jnp.exp(log_ess)
    particle_inds = ((ess < ess_threshold) * resampled_inds) + (
        (ess >= ess_threshold) * jnp.arange(len(log_weights))
    )
    # particle_inds = jnp.arange(len(log_weights))
    return particle_inds, log_total_weight, log_normalized_weights


def run_single_step_smc_jax(
    key,
    obs_trace,
    proposal,
    model,
    ...
    last_step_traces, resampled_traces, log_total_weight, log_norm_weights, pqobs = (
        prevstate_and_score
    )
    ...
    proposal_map = jax.vmap(
        lambda k, rst: proposal.propose(k, rst.get_retval() + (obs_trace.get_retval(),))
    )(subkeys, resampled_traces)

    """ EACH SAMPLE IS SCORED UNDER THE SAME ARGUMENTS IN THE GENERATIVE MODEL """
    model_map = jax.vmap(
        lambda k, propmap, rst: model.importance(
            k, cm_translator(propmap), rst.get_retval()
        )
    )(subkeys, proposal_map[0], resampled_traces)
    obs_map = jax.vmap(
        lambda k, mm: obs_model.importance(k, obs_trace.get_choices(), mm.get_retval())
    )(subkeys, model_map[0])
    p_q_obs = (model_map[1], proposal_map[1], obs_map[1])
    jax.debug.print(
        "Values: P = {p_val}, Q = {q_val}, O = {o_val}",
        p_val=p_q_obs[0],
        q_val=p_q_obs[1],
        o_val=p_q_obs[2],
    )
    ...
    key, subkey = jax.random.split(key, 2)
    resampled_particles, log_total_weight, log_norm_weights = maybe_resample(
        subkey, scores, jnp.inf
    )
    next_parent_state = jax.vmap(lambda i: index_pytree(model_map[0], i))(
        resampled_particles
    )
    return (
        model_map[0],
...
def run_particle_filter(
    obs_traces, n_particles, num_steps, genfns, pfkey, prop_to_model_cm_translator
):
    ...
    init_states_and_scores = run_single_step_smc_jax(
        init_key,
    ...
    first_step_states_and_scores = run_single_step_smc_jax(
        pf_key_firststep,
    ...
    def particle_step(states, obs_and_key):
        ...
        newstates = run_single_step_smc_jax(
            pfkey,
            obs,
            step_proposal,
            step_latent_model,
    ...
    unrolled_pf = jax.lax.scan(
        particle_step,
        first_step_states_and_scores,
        (jax.tree_map(lambda x: x[2:num_steps], obs_traces), unrolled_keys),
    )
