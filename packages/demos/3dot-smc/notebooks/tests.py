...
mod_probs = get_categorical_probs(
    key, init_mod_imp, ("ego_pos", "ego_matter"), (), CMB.d({})
)
prop_probs = get_categorical_probs(
    key,
    init_prop_imp,
    ("dot", "ego_pos", "ego_matter"),
...
assert (
    ego_constraint.value
    == tr_constr.get_choices()["dot", "ego_pos", "ego_matter"].value
).all()
...
prop_probs_20_arg_no_constraint = get_categorical_probs(
    key, init_prop_imp, ("dot", "xyz"), (vis_angle_observations[20],), CMB.d({})
)
...
q_probs = jnp.array([0.1, 0.2, 0.7])
p_probs = jnp.array([0.1, 0.2, 0.7])
probmap_q = [jnp.array([0.1, 0.9]), jnp.array([0.001, 0.999]), jnp.array([0.2, 0.8])]
probmap_p = [jnp.array([0.9, 0.1]), jnp.array([0.2, 0.8]), jnp.array([0.5, 0.5])]
...
particle.start_sampler("lights", (jnp.array([0.1, 0.9]),), 0.0)
...
particle.start_pq_scoring("lights", jnp.array([0.1, 0.9]))
particle.start_pq_scoring(("ego_pos", "ego_matter"), prop_probs)

# likelihood
print("starting likelihood scoring")
args_to_obs = tr_prop.get_retval()
obs_probs = get_categorical_probs(
    key, obs_mod_imp, ("obs", "pix"), args_to_obs, CMB.d({})
)
