...
xyz_step = 1
xs = jnp.arange(-5., 6., xyz_step)
ys = jnp.arange(ystart, ystart+10., xyz_step)
zs = jnp.arange(-5., 6., xyz_step)
xyz_point_cloud = jnp.array(
    list(itertools.product(xs, ys, zs)))
vel_step = 1
...
xyz_vel_mags = jnp.arange(-3., 3.5, vel_step)
xyz_vels = jnp.array(
    list(itertools.product(xyz_vel_mags, xyz_vel_mags, xyz_vel_mags)))
angle_div = jnp.pi/32
visual_angles = jnp.arange(-jnp.pi / 2 , jnp.pi / 2, angle_div)
egocentric_2d_map = jnp.array(
    list(itertools.product(visual_angles, visual_angles)))
objects = jnp.array([0])
bool_support = jnp.array([0, 1])
diams = jnp.arange(1., 3., .1) 
σ_ang = .05
σ_diam = .1
σ_pos = .1
σ_vel = .1
...
depth_div = 1.0
depths = jnp.arange(1, jnp.linalg.norm(jnp.array([xs[-1], ys[-1], zs[-1]])) + depth_div, depth_div) 
...
spherical_map_indices = jax.vmap(lambda egopos: jnp.where(jnp.all(round_to_support(jnp.array(spherical_to_xyz(*egopos)), xyz_point_cloud)==xyz_point_cloud, axis=1), size=1)[0][0])(egocentric_3d_map)

xyz_map_indices = jax.vmap(lambda xyz: jnp.where(jnp.all(round_to_support(jnp.array(xyz_to_spherical(*xyz)), egocentric_3d_map)==egocentric_3d_map, axis=1), size=1)[0][0])(xyz_point_cloud)
...
spherical_probabilities = jax.vmap(
    lambda tpr: probvecs_to_R3(
        ...
        disc_gauss_unnorm(tpr[2], σ_pos, depths)))(egocentric_3d_map)

def xyz_to_spherical_index(x, y, z):
    r = jnp.linalg.norm(jnp.array([x, y, z]))
    θ = jnp.atan(x / y)
    ϕ = jnp.asin(z / r)
    sph = jnp.array([round_to_support(θ, visual_angles), 
                     round_to_support(ϕ, visual_angles), round_to_support(r, depths)])
    spherical_index = jnp.where(jnp.all(egocentric_3d_map == sph, axis=1), size=1)[0][0]
    return spherical_index

xyz_to_spher_index_map = jax.vmap(lambda xyz: xyz_to_spherical_index(*xyz))(xyz_point_cloud)
...
xyz_probability_map = jax.vmap(
    lambda p: probvecs_to_R3(
        ...
        disc_gauss_unnorm(xyz_point_cloud[p][2], σ_pos, zs)))(xyz_to_spher_index_map)
...
def is_inside_grid(angs):
    θ, ϕ = angs 
    r_vec = jax.vmap(lambda theta, phi, r: out_of_boundary(theta, phi, r), in_axes=(None, None, 0))(θ, ϕ, depths)
...
def obj_to_ego_matter(xyz, radius):
    bool_xyz = jax.vmap(lambda pos: jnp.linalg.norm(pos - xyz) <= radius)(xyz_point_cloud).astype(int)
    gaussian_sums = jax.vmap(lambda b_xyz, sp_ind: b_xyz * spherical_probabilities[sp_ind], in_axes=(0,0))(bool_xyz, xyz_to_spher_index_map)
...
def egocentric_matter_map(prob_occupied):
    matter = labeled_categorical(jnp.array([1-prob_occupied, prob_occupied]), bool_support) @ "ego_matter"
    return matter
...
def step_model(vₚ, xyzₚ, vc_θϕrₚ, lightsₚ, diamₚ):
    ...
    lights_on = lightsₚ * .8 + .1
    lights_off = (1 - lightsₚ) * .8 + .1 
    lights = labeled_categorical(jnp.array([lights_off, lights_on]), 
                                 bool_support) @ "lights"
...
def compress_3D_to_2D(vc_θϕr):
    ...
    compressed_map = jax.vmap(lambda x: jnp.minimum(jnp.sum(x), 1.))(partitioned_matter_map)
    return compressed_map

@genjax.vmap(in_axes=0)
@genjax.gen
def render_to_2D(pixprob):
    obs_prob = pixprob * jnp.array([pixflip_noise, 1-pixflip_noise]) + (1-pixprob) * jnp.array([1-pixflip_noise, pixflip_noise])
...
def dig_2d_array(white_indices):
    ...
    return jax.vmap(lambda wi: ind_to_dig(wi))(jnp.arange(len(egocentric_2d_map))).astype(int)
...
def generate_pix_to_ego_2d_map(frm_indices_array, frame):
    def map_to_ego_2d(pix):
        ...
        θ = round_to_support(
            jnp.arctan((pix[0] - (x_pix / 2)) / jnp.abs(observer)), 
                            visual_angles)
        ϕ = round_to_support(
            jnp.arctan((pix[1] - (y_pix / 2)) / jnp.abs(observer)),
                            visual_angles)
        ego_ind = jnp.where(jnp.all(egocentric_2d_map == jnp.array([θ,ϕ]) ,axis=1), size=1)[0][0]
        return ego_ind
...
obs_frames = collect_quil_frames("/Users/nightcrawler/PointLightClj/saved_animation")[1:]
...
pix_to_ego_2d_map = generate_pix_to_ego_2d_map(
     frame_indices_array, obs_frames[1]).reshape(obs_frames[1].shape)

def find_occupied_2d_angles(frame):
   mask = jnp.where(frame, 1., jnp.nan) 
...
def generate_obs_traces(observations):
    ...
    def imp(ob):
        obs_tr, w = obs_model.importance(key, 
                                         CMB.d({("obs", "pix") : ob}), random_args)
        return obs_tr
...
def dig_xyz(arr):
    return jax.vmap(lambda i:
        jax.lax.cond(jnp.any(arr == i), lambda x: 1., lambda x: jnp.nan, 1))(
...
def map_2d_to_Θϕr(ego_2d_occupancy, r_μ):
    ...
    def map_2d_occupancy_to_3d_gaussian(Θϕ):
        return probvecs_to_R3(discrete_norm(Θϕ[0], σ_ang, visual_angles), 
                              discrete_norm(Θϕ[1], σ_ang, visual_angles), 
                              discrete_norm(r_μ, σ_pos, depths))
    gaussians = jax.vmap(
        lambda occ, ego2d: jax.lax.cond(
            occ, 
            map_2d_occupancy_to_3d_gaussian, 
            zero_array, 
            ego2d))(ego_2d_occupancy, egocentric_2d_map)
    probability_map = jnp.minimum(jnp.sum(gaussians, axis=0), 1.)
...
# go from 2d occupancy to 3D probs. each will be a gaussian centered on a angle-depth tuple. you make the gaussians by cycling through positive ego occupancies and making a probvecs to R3 call with discrete_norm on theta, phi, and R. Then add them up and normalize maximum to 1, and sample from the vmap over bernoullis.
    ...
    xyz_mean = round_to_support(
        jnp.nanmean(dig_xyz(xyz_occupancy)[:, None] * xyz_point_cloud, axis=0), xyz_point_cloud)
    xyz_probvec = probvecs_to_R3(
    ...
    vₜ = labeled_categorical(unicat(xyz_vels), xyz_vels) @ "v3d"
    diam = labeled_categorical(
        discrete_norm(round_to_support(sphere_diameter(jnp.sum(vc_θϕr)), diams), 
                                       σ_diam, 
                                       diams), diams) @ "diam"
    lights = labeled_categorical(jnp.array([0.0, 1.0]), bool_support) @ "lights"
    return (vₜ, xyzₜ, vc_θϕr, lights, diam)
...
def step_proposal_vis(vₚ, xyzₚ, vc_θϕrₚ, lightsₚ, diamₚ, obs):
    ...
    diam = labeled_categorical(
        discrete_norm(round_to_support(sphere_diameter(jnp.sum(vc_θϕr)), diams), 
                                       σ_diam, 
                                       diams), diams) @ "diam"
    lights = labeled_categorical(jnp.array([0.01, 0.99]), 
                                 bool_support) @ "lights"
    return (vₜ, xyzₜ, vc_θϕr, lights, diam)
...
def initial_proposal(obs):
    islit = jnp.max(obs)
    rv = genjax.switch(initial_model, initial_proposal_vis)(
        islit,
        (),
        (obs,)) @ "dot"
    return rv
...
def step_proposal(vₚ, xyzₚ, vc_θϕrₚ, lightsₚ, diamₚ, obs):
    islit = jnp.max(obs)
    rv = genjax.switch(step_model, step_proposal_vis)(
        islit,
        (vₚ, xyzₚ, vc_θϕrₚ, lightsₚ, diamₚ), 
        (vₚ, xyzₚ, vc_θϕrₚ, lightsₚ, diamₚ, obs)) @ "dot"
    return rv
...
gen_funs = [initial_proposal, initial_model,
            step_proposal, step_model, obs_model]
num_particles = 50
...
def test_assess_and_propose(obs_traces):
    ...
    init_assess = initial_model.assess(translate_proposal_cm_to_model(init_prop_tr.get_sample()), ())
    ...
    return init_prop_tr.get_sample(), init_mod_importance, init_mod_importance_w_translation


def test_obj_to_ego():
    probmap = obj_to_ego_matter(jnp.array([0.0, 5.0, 0.0]), 1.0)
    highprob_args = jnp.argwhere(probmap > .003)
...
def test_egocentric_matter_score(obs_traces):
    ...
    egomap = obj_to_ego_matter(xyz_retval, diam / 2.)
    ...
    print(egocentric_3d_map[egomap > .03][:, 0:2])
...
def animate_current_particle_locs(observations, points_3D_seq, scores, plot_history, interval=50):
...
animate_current_particle_locs(observations[0:len_sim], 
                              xyz, 
                              pscores, True)
...
model_variables = [
    ...
    {"variable": "xyz", "parents": ["v3d"], "support":xyz_point_cloud, "subtraced": []},
    {"variable": "ego_pos", "parents": ["xyz"], "support":egocentric_3d_map, "subtraced": []}
    ]

proposal_variables = [
    {"variable": "ego_pos", "parents": [], "support":egocentric_3d_map, "subtraced": []},
    {"variable": "xyz", "parents": ["ego_pos"],   "support":xyz_point_cloud, "subtraced": []},
...
def make_lfp_and_spikes(pf_results, obs):
    spikes =  merge_particle_dicts(
        snmc_spikes_wrapper(
            pf_results, 'r', range(len(obs)), 
            range(num_particles), 
            get_components(depths, range(num_particles), ['ctx']), False)[0])
    
    phase_precession_neurons = ["assemblies_p" + str(i) for i, x in enumerate(depths)]
    particle = 1
    r_spikes_particle = snmc_spikes_wrapper(
        pf_results, 'r', 
        range(len(obs)), [particle], phase_precession_neurons, False)[0]
    ...
    merge_components = merge_component_dicts(spikes, get_components(depths, range(num_particles), "ctx"))
