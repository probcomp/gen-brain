...
def gather_spikes_from_single_particle_samplescore(pf_results, particle_id, v, step_range):
    particles, resamplers = pf_results[1:3]
    particle_over_time = [p[particle_id] for p in particles[step_range[0]:step_range[-1]+1]]
    resamplers = resamplers[step_range[0]:step_range[-1]+1]
    resampler_start_and_end_times = [(r.resampler_start_time, r.resampler_end_time) for r in resamplers]
    offset_time = 0
    ...
    for step, ss in enumerate(sss):
        ...
        offset_resampler_spikes = add_offset_to_spikes(new_resampler_spikes,
                                                       resampler_offset)
        if step == 0:
            ...
            for neuron_id, (spikes, label) in samplescore_spiketimes.items():
                extended_ss_spikes = np.concatenate((spikes, offset_ss_spikes[neuron_id][0]))
                new_ss_spiketimes[neuron_id] = (extended_ss_spikes, label)
            for neuron_id, (spikes, label) in resampler_spiketimes.items():
                extended_rs_spikes = np.concatenate((spikes, offset_resampler_spikes[neuron_id][0]))
...
def my_tab20(num_colors):
    ...
    new_20 = [sns.color_palette("tab20b", num_colors)[i] for i in particle_colors+non_particle_colors]
    return new_20
...
#    cpal = sns.color_palette("tab20b", 20)
    ...
    if resampler != ():
        ...
        indexed_resampler_spikes = {i + len(ss_spiketimes_list[0]) : v for i, v in resampler_spikedict.items() }
    ...
    for c, spiketimes in enumerate(ss_spiketimes_list):
        for neuron_id, spikes_and_label in spiketimes.items():
            ...
            if label[0:3] in ["res", "nor"]:
                ...
                color_id = c
            ax.vlines(spikes, neuron_y, neuron_y + .8, color=cpal[color_id], linewidth=1.0)
...
def animate_snmc_spikes(spiketimes):
    ...
    lines = [ax.vlines([], [], [], color='darkcyan', linewidth=1.5) for _ in range(num_neurons)]
...
#        Calculate the time window to display (most recent 10 seconds)
        ...
        for neuron_id, spikes_and_label in spiketimes.items():
            ...
            visible_spikes = spikes[(spikes >= time_window_start) & (spikes <= time_window_end)]
            # Update the data in vlines for the corresponding neuron
            lines[neuron_id].set_segments([[[spike - time_window_start, neuron_y], [spike - time_window_start, neuron_y + .8]] for spike in visible_spikes])
    ...
    frames_total = np.max(np.concatenate([v[0] for v in spiketimes.values() if not len(v[0]) == 0])).astype(int)
    ani = FuncAnimation(fig, update, interval=40,
                        frames=np.arange(0, (frames_total+win) / 10, 1), repeat=True, blit=False)
    return ani
    


def snmc_spikes_wrapper(pf_results, v, step_range, particles_to_plot, components_to_plot, plotit):
    all_ss_component_spiketimes = []
    for p_i, p in enumerate(particles_to_plot):
        ss_raw, rs = gather_spikes_from_single_particle_samplescore(pf_results, p, v, step_range)

        ss = organize_labels_into_layers(ss_raw)
        if components_to_plot != ():
            ss_spikearrays = [v for i, (k, v) in enumerate(ss.items()) if v[1] in components_to_plot]
            ss_component_spiketimes = { i : v for i, v in enumerate(ss_spikearrays) }
            if p_i == 0:
                rs_spikearrays = [v for i, (k, v) in enumerate(rs.items()) if v[1] in components_to_plot]
...
def plot_particle_weight_and_state(pf_results, v, step_range):
    ...
    for step, (particles_at_step, resampler_at_step) in enumerate(
        ...
        # if (particle_prob == -np.inf).all():
        #     particle_prob = normalize(np.ones(len(particle_prob)))
        # elif (particle_prob == -np.inf).any():
        #     particle_prob = [pp if np.isfinite(pp) else 0 for pp in particle_prob]
        offset = .1
        size_scale = 50
        for p, (size, y, color) in enumerate(zip(particle_probs,
                                                 particle_state, marker_colors)):
...
#    resfactor = 1
    binsize = 1
    spikes = [np.round(v[0], binsize) for k, v in spiketimes.items()] #if v[1] in filterfunc]
...
def variability_quench(pf_results, variable, components):
    ...
    for step in range(num_total_steps):
        ss_spiketimes, res_spiketimes = snmc_spikes_wrapper(pf_results,
                                                            variable,
                                                            range(step, step+1), 
                                                            range(num_particles), components)
...
def lfp_and_spikes(spiketimes, lfp, plot_now):
    spike_height = 1
    lfp_height = 4
    neuron_y = lfp_height + .2
    lfp_scaled = (lfp_height / np.max(lfp)) * lfp
    #fig_hist, ax_hist = plt.subplots(len(spiketimes), 1)
    cpal = my_tab20(5)
    curr_label = ""
    components = []
    if plot_now:
        ...
        for i, (k, (spikes, component)) in enumerate(spiketimes.items()):
            ...
            if component[0:5] == "assem":
                ...
                clr = cpal[2]
            ax.vlines(spikes, neuron_y, neuron_y + .8*spike_height, color=clr, linewidth=1.0)
...
def selectivity_index(pf_results, particle_id, variable):
    ...
    # rates.
    samplescores_per_step = [pf_results[1][step][particle_id].samplescores[variable] for step in range(len(pf_results[1]))]
    spikes_per_step = [get_spikes_from_samplescore(ss) for ss in samplescores_per_step]
    num_neurons = len(spikes_per_step[1])
    single_neuron_over_time = [[spikes[neuron] for spikes in spikes_per_step] for neuron in range(num_neurons)]
    labels_and_counts_for_neurons_that_spiked = [(spikes[0][1], [len(sp[0]) for sp in spikes]) for spikes in single_neuron_over_time if np.concatenate([s[0] for s in spikes]).size > 0]
   ...
   # return labels_and_counts_for_neurons_that_spiked
    ...
    preference_index = list(map(lambda x: p_i(x),
...
def direction_selectivity(pf_res1, pf_res2, step, variable):
    ...
    for neuron_l, neuron_r in zip(spikes_l.values(), spikes_r.values()):
        ...
        if neuron_l[0].size == 0 and neuron_r[0].size == 0:            
            ...
            ds_index = (len(neuron_l[0])- len(neuron_r[0])) / (len(neuron_l[0]) + len(neuron_r[0]))
...
# Create custom legend handles
    ...
    def update(frame):
        ...
        for spikes, neuron_y, color in spike_lines:
            ...
            ax.vlines(active_spikes, neuron_y, 
                      neuron_y + .8 * spike_height, color=color, linewidth=1.0)
    ...
    ani = FuncAnimation(fig, update, frames=np.arange(0, len(lfp_scaled)), interval=interval, blit=False)
    return ani
